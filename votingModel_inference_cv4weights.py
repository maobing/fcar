...
#   1. run splitData_cv4.py for feature files, usually 3 fold
#   2. run votingModel_inferece.py for cv1,cv2,cv3
#   3. then run this file, as this file will read in
#     cv\d+ results.
#----------------------------------------
# 2015-05-03 BH
# @UPDATE: when calculating weights,
#   use cross-validated errors
# model stacking
...
# getVotingWeights
#---
def getVotingWeights(trainFile, model, fold = 3) :
  print '------------------------------------'
  ...
  # read in cv results
  for i in range(fold):
    cvTrainFilename = trainFile + '_cv' + repr(i+1) + '_train'
    cvTestFilename = trainFile + '_cv' + repr(i+1) + '_test'
    tmpmodel = list(set([ a.split('_')[0] for a in model]))
    cvResultFilename = ''.join( ['../',os.path.basename(cvTrainFilename), \
      os.path.basename(cvTestFilename),','.join(tmpmodel), ',voting','_result'] )
    print 'opening files %s' % cvResultFilename
    tmpFp = open(cvResultFilename, 'r')
    for line in tmpFp:
      tmpTerms = line.split()
      ys.append(int(tmpTerms[0]))
      result.append( [ float(x.split(':')[1])  for x in tmpTerms[1:]] )
    tmpFp.close()
  ...
  for j in range(len(result[0])) :
    ...
    for k in range(len(result)) :
      tmpPred.append(result[k][j])
    result2.append(tmpPred)

  errs = [ err(yhat, ys) for yhat in result2 ]
...
def writeFormatted(ys, result, outputFile) :
  ...
  print '- len(result[0]) is ', len(result[0])
    ...
    for j in range(len(result)) :
      if j == len(result) - 1 :
        ...
        # try:
          # fp.write(''.join( [repr(j+1),':', repr(result[j][i]), '\n'] ))
        # except:
          # print 'i is %d, j is %d, result[j] is %f' % (i,j,result[j])
      else :
        fp.write(''.join( [repr(j+1),':', repr(result[j][i]), ' '] ))
  return 0
...
def main(argv) :
  ...
  weights = getVotingWeights(trainFile, model)
  top = 3
  votedResult = getVotingResults(testResult, weights[:(len(weights)-1)], top)
  votedResultControl = getVotingResults(controlResult, weights[:(len(weights)-1)], top)
